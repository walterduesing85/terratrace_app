...
final weightedLatLngListProvider =
  ...
  final dataList = await ref.watch(fluxDataListProvider.future);
  final intensities = await ref.watch(intensityProvider.future);

  // Erzeuge eine lokale Liste, um die Änderungen zu speichern
  List<WeightedLatLng> weightedLatLngList = [];

  for (int i = 0; i < dataList.length; i++) {
    weightedLatLngList.add(WeightedLatLng(
      point: LatLng(double.parse(dataList[i].dataLat),
          double.parse(dataList[i].dataLong)),
  ...
  Future<void> initHeatmap(WidgetRef ref) async {
    try {
      ...
      if (weightedLatLngList.isNotEmpty) {
        ...
        print("Error: weightedLatLngList is empty init HeatMap");
      }
    } catch (e) {
...
final initialCameraPositionProvider = StateProvider<LatLng>((ref) {
  ...
  LatLng defaultPosition = LatLng(52.4894, 13.4381);
